...
```{r}
library(tidyverse)
coef(fit1$params$theta, simplify = TRUE)

fit1$params$theta[1, , , 1]

df <- as.data.frame(fit1$params$theta[1, , , 1]) |>
  pivot_longer(everything(), names_to = 'network',
               values_to = 'edges_posterior') 
  
df |> 
  ggplot(aes(x = edges_posterior))+
  geom_density(aes(color = network, fill = network,
                   alpha = 0.05))+
...
parameter $\mu$, then simulate a network from each value. By comparing these
...
The algorithm used to produce posterior samples is fairly computationally expensive. To reduce the computational burden, we suggest initialising theMarkov chain with sensible values, and adjusting the size of the MCMC proposals.
...
```{r}
fit2 <- multibergm(krackfr$networks ~ edges, init = init, groups = c(rep(1, times = 10), rep(2, times = 11)))
