...
gof.multibergm <- function(object,
  ...
  # added so mapply works below
  if (is.null(object$groups)) {
    object$groups <- rep(1, times = length(object$networks))
    #object$groups <- seq(1, length(object$networks), 1)
  }
  ...
  if (is.null(coefs)) {
    ...
    output <- assign(param, object$params) 
    #get(param, object$params)
    
    n_iters <- mcmcr::niters(object$params$theta)
    coefs   <- subset(output, iters = sample(n_iters, sample_size))
    coefs   <- abind::adrop(unclass(coefs)$theta, 1)
      #abind::adrop(unclass(coefs), 1)
  }

  sim_df <- obs_df[0, ]
  for (g in unique(object$groups)) {
    group_nets <- object$networks[unlist(sapply(object$networks,
                         function(x) get.network.attribute(x, 'group') == g,
                         simplify = FALSE))] #maybe fix for more flexibility
      
      #object$networks[object$groups == g]; old version
    ...
    for (i in seq_len(sample_size)) {
      ...
      # where i left off!!! need to fix to handle mcmcarray
  ...
  sim_df <- sim_df %>%
    ...
    dplyr::summarise(Median = quantile(Value, 0.5),
              ...
              Upper = quantile(Value, 0.95), .groups = 'keep')
  ...
  if (is.directed(object$networks[[1]])) {
    #return(lapply(list(p0, p1, p2, p3), print))
    cowplot::plot_grid(p0, p1, p2, p3, ncol = 2)
  } else {
...
geodist_gof <- function(obs_df, sim_df) {
  ...
  p1 <- ggplot(obs_df, aes(x = .data$n, y = .data$Value, group = .data$n)) +
    ...
    geom_line(data = sim_df, aes(y = .data$Group, x = .data$n),
              inherit.aes = FALSE, color = 'skyblue', linewidth = 1) +
    geom_ribbon(data = sim_df,
                aes(ymin = .data$Lower, ymax = .data$Upper, x = .data$n),
                alpha = 0.2, inherit.aes = FALSE, fill = 'skyblue') +
...
plot_dist_gof <- function(obs_df, sim_df, name) {
  ...
  if (n_groups == 1) {
    ggplot(obs_df, aes(x = .data$n, y = .data$Value, group = .data$n)) +
      geom_boxplot() +
      geom_line(data = sim_df, aes(y = .data$Median, x = .data$n),
                inherit.aes = FALSE, color = 'skyblue', linewidth = 1) +
      geom_ribbon(data = sim_df,
                  aes(ymin = .data$Lower, ymax = .data$Upper, x = .data$n),
                  alpha = 0.2, inherit.aes = FALSE, fill = 'skyblue') +
    ...
    ggplot(obs_df, aes(x = .data$n, y = .data$Value, group = .data$n)) +
      geom_boxplot() +
      geom_line(data = sim_df, aes(y = .data$Median, x = .data$n, color = .data$group),
                inherit.aes = FALSE, color = 'skyblue') +
      geom_ribbon(data = sim_df,
                  aes(ymin = .data$Lower, ymax = .data$Upper, x = .data$n,
                      fill = .data$group),
                  alpha = 0.2, inherit.aes = FALSE, fill = 'skyblue') +
